  ...
  <div>
    ...
    <div class="list_content">
      <div style="cursor: pointer;margin-bottom: 10px;height:40px;">
        <el-button type="primary" @click="daochu" style="float: right;margin-right: 100px;" v-if="auth.indexOf('bigCard_export') > -1">导出列表</el-button>
      </div>
    </div>
    <div>
      <el-table
        ...
        <el-table-column
          ...
          show-overflow-tooltip>
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="chakan(scope.row.userId)" v-if="auth.indexOf('bigCard_detail') > -1">查看</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div class="footers">
...
export default {
  ...
  methods: {
    ...
    // 查看
    chakan (userId) {
      this.$router.push({path: '/bigcarddetail', query: {userId: userId}})
    },
    // 页面条数修改
    sizeChange (val) {
      this.bars = val
    ...
    daochu () {
      fetch({
        ...
        success: (res) => {
          if (res.status === 200) {
            tool.download(res.data, '建档-大卡.xls', 'application/vnd.ms-excel;charset=utf-8')
            this.$notify({
              title: '导出成功',
              type: 'success'
            })
